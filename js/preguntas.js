...
function sendWhatsAppMessage(message) {
    const phoneNumber = '51937514867'; // Número de WhatsApp
    const encodedMessage = encodeURIComponent(message);
    const whatsappURL = `https://wa.me/${phoneNumber}?text=${encodedMessage}`;
...
// WhatsApp Chat Box Functions
document.addEventListener('DOMContentLoaded', function() {
    const whatsChat = document.getElementById('whats-chat');
    const chatTopRight = document.getElementById('chat-top-right');
    const sendBtn = document.getElementById('send-btn');
    const whatsInput = document.getElementById('whats-in');
    
    if (whatsChat) {
        whatsChat.addEventListener("click", showchatbox);
    }
    if (chatTopRight) {
        chatTopRight.addEventListener("click", closechatbox);
    }
    if (sendBtn) {
...
function sendmsg(){
    ...
    window.open('https://api.whatsapp.com/send?phone=51937514867&text=' + relmsg, '_blank');
